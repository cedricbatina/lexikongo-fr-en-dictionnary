...
export default defineEventHandler(async (event) => {
  ...
  try {
    console.log("[/api/all-words-verbs] handler called");
    ...
    const [results] = await connection.execute(`
      (SELECT 
          ...
          (SELECT GROUP_CONCAT(wm.meaning SEPARATOR ', ')
           ...
           WHERE wm.word_id = w.word_id
             AND wm.language_code = 'fr') AS translation_fr,
          (SELECT GROUP_CONCAT(wm.meaning SEPARATOR ', ')
           FROM word_meanings wm
           WHERE wm.word_id = w.word_id
             AND wm.language_code = 'en') AS translation_en
       FROM words w
      ...
      (SELECT 
          ...
          (SELECT GROUP_CONCAT(vm.meaning SEPARATOR ', ')
           ...
           WHERE vm.verb_id = v.verb_id
             AND vm.language_code = 'fr') AS translation_fr,
          (SELECT GROUP_CONCAT(vm.meaning SEPARATOR ', ')
           FROM verb_meanings vm
           WHERE vm.verb_id = v.verb_id
             AND vm.language_code = 'en') AS translation_en
       FROM verbs v
       JOIN slugs s ON v.verb_id = s.verb_id
      )
      ORDER BY singular COLLATE utf8mb4_unicode_ci ASC;
    `);

    console.log(
      "[/api/all-words-verbs] rows returned:",
      Array.isArray(results) ? results.length : "??"
    );

    // On renvoie directement le tableau (mots + verbes)
    return results;
  } catch (error) {
    console.error(
      "[/api/all-words-verbs] Erreur lors de la récupération des mots et verbes :",
      error
    );

    // pour bien signaler l’erreur au client
    throw createError({
      statusCode: 500,
    ...
    if (connection) {
      try {
        if (typeof connection.release === "function") {
          connection.release(); // ✅ bonne pratique avec un pool
        } else if (typeof connection.end === "function") {
          await connection.end();
        }
      } catch (e) {
        console.error(
          "[/api/all-words-verbs] Erreur lors de la fermeture de la connexion :",
          e
        );
      }
    }
